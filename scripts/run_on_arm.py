...
# Placeholder — tune on the real arm.
Q_GRAB  = _canonical(np.array([322.0,  76, 180.0, 254.0, 0.0,  94.0, 88.0]))
...
def _plan_steps(args, constraints):
    ...
    # Forward: Q_START → Q_WAYPOINT → Q_GOAL → Q_GRAB. Planners that support
    # waypoints (min-jerk) flow through interior waypoints without stopping;
    # others stop at each one and we splice the legs together.
    if hasattr(planner, "plan_waypoints"):
        forward = planner.plan_waypoints([Q_START, wp, goal, grab], constraints)
        print(f"  Forward   : {forward.duration:.3f} s  "
              f"({len(forward.t)} steps, stitched through 4 waypoints)")
    else:
    ...
    # Return: Q_GRAB → Q_START. Single rest-to-rest move — no intermediate
    # waypoints needed, so just use plan() regardless of planner.
